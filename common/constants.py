UserNotFound = "User not found or not valid"
InvalidPayload = "Invalid payload"
MissingClientKey = "Client key is not present"
ClientNotFound = "Client is not found"
AccessTokenNotFound = "Access token is not found"
SignatureNotValid = "Signature not valid"
SignatureOrSSONotPresent = (
    "Please return to your partner homepage and do login from there."
)
GeneralError = "Please try again in a moment"
RedirectionNotPresent = "Redirection URL not present"
EmailorPasswordNotValid = "Email or Password is not found or valid"
